...
def load_and_preproc(image_bytes, class_dict):
    """ Load and process image from disk
    ...
    return {'raw_bytes':image_bytes}, labels


def get_batch_loader_tfrecord(metadata, batch_size, epochs, class_dict, batch_prefech=5):
    ...
    def _build_dict(parsed_features):
        ...
        return load_and_preproc(parsed_features['image_bytes'], class_dict_tensor)

    dataset = tf.data.TFRecordDataset(metadata, compression_type="GZIP")
    dataset = dataset.map(_parse_function)    
    dataset = dataset.map(_build_dict)
